...
|`MemoryConsolidator` (`lib/memory/consolidate/MemoryConsolidator.js`)|cycle trigger|stage 결과 22항목 + per-stage timing|stage별 query error는 stage 결과에 잡힘, 다음 stage 진행|`MEMENTO_CONSOLIDATE_*`, `MEMENTO_CONSOLIDATE_TIMEOUT_MS`|`memento_consolidation_stage_ms`|미사용 (논리 변경만)|
