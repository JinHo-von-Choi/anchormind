...
| MEMENTO_CONTEXT_ANCHOR_LIMIT | 20 | Overall maximum number of anchor (isAnchor) fragments always included in context responses. Clamped to 1-30; falls back to 20 on parse failure. Anchors are not trimmed by tokenBudget, so this count cap is the only injection limit |
| MEMENTO_CONTEXT_WORKSPACE_ANCHOR_RESERVE | 10 | Slots reserved first for the highest-importance anchors in the effective workspace. Must be an integer from 0 through `MEMENTO_CONTEXT_ANCHOR_LIMIT`; invalid values fail server startup validation. Not applied when no workspace is effective |

When lowering `MEMENTO_CONTEXT_ANCHOR_LIMIT` below 10, lower the reserve as well. For example, a total limit of 5 requires an explicit `MEMENTO_CONTEXT_WORKSPACE_ANCHOR_RESERVE=5` or lower.
