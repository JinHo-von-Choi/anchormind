...
| MEMENTO_CONTEXT_ANCHOR_LIMIT | 20 | context 응답에 항상 포함되는 앵커(isAnchor) 파편의 전체 최대 개수. 1~30 범위로 클램프되며 파싱 실패 시 20. 앵커는 tokenBudget 절삭 대상이 아니므로 이 개수 상한이 유일한 주입량 제한이다 |
| MEMENTO_CONTEXT_WORKSPACE_ANCHOR_RESERVE | 10 | effective workspace가 있는 context에서 해당 workspace의 importance 상위 anchor에 먼저 예약할 슬롯 수. 0 이상 `MEMENTO_CONTEXT_ANCHOR_LIMIT` 이하의 정수여야 하며, 잘못된 값은 서버 기동 검증에서 실패한다. workspace가 없으면 적용하지 않는다 |

`MEMENTO_CONTEXT_ANCHOR_LIMIT`를 10 미만으로 낮출 때는 reserve도 함께 낮춰야 한다. 예를 들어 total을 5로 설정하면 `MEMENTO_CONTEXT_WORKSPACE_ANCHOR_RESERVE=5` 이하를 명시해야 한다.
