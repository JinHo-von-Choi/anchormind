...
server.js  (HTTP 서버)
    ...
    └── lib/memory/
            ...
            │   ├── ContextBuilder.js     context() 로직 전담. `build()` 내부에서 `#loadCoreMemory` / `#loadWorkingMemory` / `#loadAnchorMemory` / `#loadLearningFragments` / `#buildInjectionLines` / `#buildStructuredResponse` 6개 비공개 메서드로 분해. Anchor는 effective workspace 상위 예약분을 먼저 고정하고 남은 슬롯을 잔여 workspace/global 통합 랭킹으로 채운다
