...
HNSW 벡터 인덱스는 `embedding IS NOT NULL` 조건부 인덱스로 생성된다. 파라미터: m=16(이웃 연결 수), ef_construction=128(인덱스 구축 탐색 깊이), 거리 함수 vector_cosine_ops. ef_search는 `MEMORY_CONFIG.semanticSearch.hnswEfSearch` 기본값 80을 세션 레벨 SET LOCAL로 적용한다. L3 검색은 caller deadline(`timeoutMs`, 기본 5000ms)과 PostgreSQL `statement_timeout`(`statementTimeoutMs`, 기본 4500ms)을 함께 사용해 pgvector tail latency가 전체 recall을 장시간 붙잡지 못하게 제한한다.
...
**RRF 하이브리드 병합.** `text` 파라미터가 있을 때 L2와 L3는 `Promise.all`로 병렬 실행된다. 결과는 Reciprocal Rank Fusion(RRF)으로 병합된다: `score(f) = Σ w/(k + rank + 1)`, 기본값 k=60. 기본 가중치는 L1 2.0, L2 1.0, Graph 1.5, L3 1.0이다. `text`와 keywords/topic/type 중 하나 이상이 함께 들어온 mixed 경로는 키워드 신호를 보존하기 위해 L1 2.5, L2 1.7, L3 0.6을 적용한다. text-only 경로는 기본 L3 가중치를 유지한다. L1에만 있고 content 필드가 없는 파편(내용 미로드)은 최종 결과에서 제외된다. `text` 파라미터 없이 keywords/topic/type만 사용하면 L3 없이 L1+L2 결과만으로 응답한다.
