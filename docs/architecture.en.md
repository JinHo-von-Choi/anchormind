...
The HNSW vector index is created as a conditional index on `embedding IS NOT NULL`. Parameters: m=16 (neighbor connections), ef_construction=128 (index build search depth), distance function vector_cosine_ops. ef_search comes from `MEMORY_CONFIG.semanticSearch.hnswEfSearch` and defaults to 80 via session-level SET LOCAL. L3 search uses both a caller deadline (`timeoutMs`, default 5000ms) and PostgreSQL `statement_timeout` (`statementTimeoutMs`, default 4500ms) so pgvector tail latency cannot hold the entire recall path for tens of seconds.
...
**RRF hybrid merge.** When the `text` parameter is present, L2 and L3 run in parallel via `Promise.all`. Results are merged using Reciprocal Rank Fusion (RRF): `score(f) = sum w/(k + rank + 1)`, default k=60. Default weights are L1 2.0, L2 1.0, Graph 1.5, and L3 1.0. Mixed queries that combine `text` with at least one keywords/topic/type signal use L1 2.5, L2 1.7, and L3 0.6 to preserve explicit keyword relevance. Text-only queries keep the default L3 weight. Fragments that exist only in L1 and lack a content field (content not loaded) are excluded from final results. When only keywords/topic/type are used without the `text` parameter, the response contains only L1+L2 results without L3.
