...
describe("validateMemoryConfig", () => {
  ...
  it("semanticSearch statementTimeoutMs는 timeoutMs보다 낮아야 한다", () => {
    const bad = structuredClone(MEMORY_CONFIG);
    bad.semanticSearch.timeoutMs = 5000;
    bad.semanticSearch.statementTimeoutMs = 5000;
    assert.throws(() => validateMemoryConfig(bad), /statementTimeoutMs.*lower than semanticSearch\.timeoutMs/i);
  });
});
