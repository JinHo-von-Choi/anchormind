...
지원 버전: ${SUPPORTED_PROTOCOL_VERSIONS.join(", ")}`;
    ...
    const toolCount = getToolsDefinition(null, true).length;
    const result = {
...
export function handleToolsList(_params, sessionData) {
  ...
  try {
    ...
    const allTools       = getToolsDefinition(keyId, isMaster);
    const tools          = filterTools(allTools, mode, isMaster).filter(tool =>
...
export function applyTrustedToolContext(params, sessionData) {
  ...
  if (!identityConsistent) {
    const error = new Error(
      "Authenticated non-master session requires an API-key binding and permission context"
    );
    error.code = -32001;
    throw error;
  }
