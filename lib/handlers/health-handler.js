...
import { ACCESS_KEY, AUTH_DISABLED, REDIS_ENABLED } from "../config.js";
...
export function isHealthRequestAuthorized({ accessKey = ACCESS_KEY, authDisabled = AUTH_DISABLED, req }) {
  if (!accessKey) return authDisabled === true;
...
export async function handleHealth(req, res, startTime) {
  const isAuthenticated = isHealthRequestAuthorized({ req });
  if (!isAuthenticated) {
    sendUnauthorized(res);
    const duration = Number(process.hrtime.bigint() - startTime) / 1e9;
    recordHttpRequest(req.method, "/health", 401, duration);
    return;
  }
...
export async function handleMetrics(req, res, startTime) {
  if (!isHealthRequestAuthorized({ req })) {
    sendUnauthorized(res);
    const duration = Number(process.hrtime.bigint() - startTime) / 1e9;
