...
export class ClaimStore {
  ...
  async findPolarityConflicts(fragmentId, keyId = null, opts = {}) {
    ...
    const scope = { keyId: normalizeKeyId(keyId), workspace: opts.workspace ?? null };
    const fromScope = strictScope ? exactScopeClause(params, "f1", scope) : "";
    const toScope = strictScope ? exactScopeClause(params, "f2", scope) : "";
    ...
    const sql = `
      SELECT c1.fragment_id AS f1,
        ...
        ${strictScope ? `JOIN agent_memory.fragments f1 ON f1.id = c1.fragment_id
        JOIN agent_memory.fragments f2 ON f2.id = c2.fragment_id` : ""}
