...
export function startSchedulers({ globalEmbeddingWorkerRef }) {
  if (isSecurityPilotAutomationOff()) {
    logWarn("[Scheduler] disabled by MEMENTO_SECURITY_PILOT_AUTOMATION=off");
    return { started: false, reason: "security_pilot_automation_off" };
  }
  /** 세션 정리 (5분) */
  ...
  import("./memory/embedding/EmbeddingWorker.js")
    ...
    .then(async (worker) => {
      worker.on("embedding_ready", async ({ fragmentId }) => {
        const reg = getSchedulerRegistry();
        /** embedding_ready currently carries no authoritative key/workspace tuple. */
        logWarn(`[GraphLinker] skipped ${fragmentId}: exact key/workspace scope unavailable`);
        reg.recordSuccess("graphLinker", { fragmentId, linked: 0, skipped: "missing_exact_scope" });
      });
    })
    .catch(err => {
