...
export const SECURITY_PILOT_MUTATION_FLAGS = Object.freeze([
  "ENABLE_SPREADING_ACTIVATION",
  "ENABLE_RECONSOLIDATION",
  "MEMENTO_AUTO_REFLECT",
  "MEMENTO_GRAPH_LINK",
  "MEMENTO_CONSOLIDATE",
  "MEMENTO_GC",
  "MEMENTO_CONSOLIDATE_SPLIT_LONG",
  "MEMENTO_CONSOLIDATE_DETECT_CONTRADICT",
  "MEMENTO_CONSOLIDATE_COMPRESS_OLD"
]);
...
export function validateSecurityPilotStartup(env = process.env, { exists = existsSync } = {}) {
  ...
  for (const name of SECURITY_PILOT_MUTATION_FLAGS) {
    if (String(env[name] ?? "").trim().toLowerCase() !== "false") {
      return fail(`${name}=false is required for the security pilot`);
    }
  }
