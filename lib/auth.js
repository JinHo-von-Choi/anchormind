...
export async function validateAuthentication(req, msg) {
  if (!ACCESS_KEY) {
    const emptyKeyDecision = buildAuthDecision(ACCESS_KEY, AUTH_DISABLED, null);
    if (!emptyKeyDecision.valid) {
      recordAuthDenied(emptyKeyDecision.error || "access_key_required");
      return emptyKeyDecision;
    }
    return { valid: true, keyId: null, groupKeyIds: null, authDisabled: true };
  }
