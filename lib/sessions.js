...
import { isAutoReflectDisabled } from "./security-pilot.js";
...
/**
 * AutoReflect에 전달할 세션의 immutable 인증 범위.
 * 반환값은 인증 키가 없는 master/legacy면 null, 인증 키가 있으나 범위가
 * 불완전하면 undefined, 완전한 API-key scope면 immutable tuple이다.
 */
export function getSessionReflectScope(session) {
  const keyId = session?.keyId;
  const workspace = session?.defaultWorkspace;
  const groupKeyIds = session?.groupKeyIds;
  if (keyId == null) return null;
  if (typeof keyId !== "string" || keyId.length === 0
      || typeof workspace !== "string" || workspace.length === 0
      || !Array.isArray(groupKeyIds) || groupKeyIds.length === 0
      || groupKeyIds.some(id => typeof id !== "string" || id.length === 0)) {
    return undefined;
  }
  return { keyId, groupKeyIds: [...new Set(groupKeyIds)], workspace };
}

async function autoReflectSession(sessionId, session, autoReflectFn = autoReflect) {
  if (isAutoReflectDisabled()) return null;
  const scope = getSessionReflectScope(session);
  if (scope === undefined) return null;
  if (scope === null) return autoReflectFn(reflectTargetId(session, sessionId));
  return autoReflectFn(reflectTargetId(session, sessionId), "default", scope);
}
 ...
 */
export async function currentSegmentId(sessionId, { autoReflectFn = autoReflect } = {}) {
  ...
  if (previousSeq != null) {
    const scope = getSessionReflectScope(session);
    if (!isAutoReflectDisabled()) {
      if (scope === null) autoReflectFn(`${sessionId}#${previousSeq}`).catch(() => {});
      else if (scope !== undefined) autoReflectFn(`${sessionId}#${previousSeq}`, "default", scope).catch(() => {});
    }
  }
 ...
 */
export async function closeStreamableSession(sessionId, { preserveRedis = false, autoReflectFn = autoReflect } = {}) {
  const session             = streamableSessions.get(sessionId);

  if (!session) return;

  /** 세션 종료 전 자동 reflect (비차단: 실패해도 세션은 닫힘) — 현재 세그먼트 대상 */
  try { await autoReflectSession(sessionId, session, autoReflectFn); } catch { /* noop */ }
 ...
 */
export async function closeLegacySseSession(sessionId, { preserveRedis = false, autoReflectFn = autoReflect } = {}) {
  const session             = legacySseSessions.get(sessionId);
  ...
  /** Legacy SSE has no authenticated key, so preserve its historical reflect path. */
  if (!isAutoReflectDisabled()) {
    try { await autoReflectFn(sessionId); } catch { /* noop */ }
  }
 ...
 */
export async function cleanupExpiredSessions({ autoReflectFn = autoReflect } = {}) {
  const now                   = Date.now();
  ...
  for (const [sessionId, session] of streamableSessions.entries()) {
    if (now > session.expiresAt) {
      ...
      await closeStreamableSession(sessionId, { autoReflectFn });
      streamableExpired++;
    } else {
      ...
      if (!isAutoReflectDisabled() && isIdleEnough && needsReflect) {
        try {
          await autoReflectSession(sessionId, session, autoReflectFn);
          if (getSessionReflectScope(session) === undefined) continue;
  ...
  for (const [sessionId, session] of legacySseSessions.entries()) {
    if (now > session.expiresAt) {
      ...
      const idleMin = Math.round((now - (session.lastAccessedAt || session.createdAt)) / 60000);
      logInfo(`[Session] Expired: ${sessionId.slice(0, 8)}... age=${ageMin}min idle=${idleMin}min`);
      await closeLegacySseSession(sessionId, { autoReflectFn });
      legacyExpired++;
    }
  }
