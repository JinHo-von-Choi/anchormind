 ...
 * key/workspace/group 중 하나라도 없으면 master/unrestricted reflect를 하지 않는다.
 */
...
export async function currentSegmentId(sessionId) {
  ...
  if (previousSeq != null) {
    const scope = getSessionReflectScope(session);
    if (scope) autoReflect(`${sessionId}#${previousSeq}`, "default", scope).catch(() => {});
  }
    ...
    if (now > session.expiresAt) {
      ...
      if (isIdleEnough && needsReflect) {
        try {
          ...
          if (!getSessionReflectScope(session)) continue;
