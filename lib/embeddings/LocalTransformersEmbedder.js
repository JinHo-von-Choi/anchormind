...
import { isOfflineModelMode, resolveSecurityPilotModelPaths } from "../security-pilot.js";
...
export class LocalTransformersEmbedder {
  ...
  async init() {
    ...
    if (!this._initPromise) {
      ...
      const offline = isOfflineModelMode();
      const modelPaths = resolveSecurityPilotModelPaths(process.env);
      const cacheDir = modelPaths.cacheRoot;
      if (offline) {
        if (transformers.env) transformers.env.allowRemoteModels = false;
        if (transformers.env) transformers.env.allowLocalModels = true;
        if (process.env.SECURITY_PILOT_MODEL_SNAPSHOT) {
          if (transformers.env) transformers.env.localModelPath = process.env.SECURITY_PILOT_MODEL_SNAPSHOT;
        }
        if (cacheDir && transformers.env) transformers.env.cacheDir = cacheDir;
      }
      const options = offline
        ? { dtype: "q8", local_files_only: true, allowRemoteModels: false, ...(cacheDir ? { cache_dir: cacheDir } : {}) }
        : { dtype: "q8" };
      this._initPromise = transformers.pipeline("feature-extraction", this.modelId, options)
        .then((p) => {
          this._pipeline = p;
