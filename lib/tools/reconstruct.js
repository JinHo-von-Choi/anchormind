...
import { exactScopeClause, keyScopeClause } from "../memory/keyScope.js";
 ...
 * @private
 */
async function _queryFragmentTraces({ event_type, entity_key, keyword, case_id, session_id, time_range, limit, keyId, groupKeyIds, workspace, strictScope = false }) {
  ...
  /** API-key pilot: exact scalar key/workspace; workspace-only keeps legacy behavior. */
  let keyFilter = "";
  if (strictScope) {
    keyFilter = exactScopeClause(params, "f", { keyId, workspace });
  } else {
    keyFilter = keyScopeClause(params, "f.key_id", { keyId, groupKeyIds });
  }
  ...
  const sql = `
    SELECT f.id, f.content, f.type, f.topic, f.case_id, f.session_id,
     ...
     WHERE ($${caseIdx}::text  IS NULL OR f.case_id    = $${caseIdx})
       ...
       ${keyFilter}
       ${strictScope ? "" : `AND ($${wsIdx}::text IS NULL OR f.workspace = $${wsIdx} OR f.workspace IS NULL)`}
  ...
  return strictScope
    ? rows.filter(row => row?.key_id === keyId && row?.workspace === workspace)
    : rows;
}
