...
import { assertAllWorkspacesAuthorized, resolveWorkspaceScope } from "../memory/read/WorkspaceScope.js";
import { appendWorkspaceCondition } from "../memory/read/WorkspaceScope.js";
...
export async function tool_recall(args) {
  ...
  try {
    ...
    const linkedMap   = await fetchLinkedFragments(fragmentIds, {
      ...
      includePeerAgents: args.includePeerAgents === true,
      ...workspaceScope
    }).catch((err) => {
    ...
    if (args.includeContext) {
      ...
      const sessionResults = await Promise.all(
        sessionIds.map(sid => mgr.store.searchBySource(
          `session:${sid}`, agentId, keyId, 5,
          workspaceScope.workspace, workspaceScope.allWorkspaces
        ))
      );
      ...
      const stitchScope = {
        ...
        allWorkspaces: workspaceScope.allWorkspaces
      };
...
async function hasPendingContradictions(fragmentIds, scope = {}) {
  ...
  try {
    ...
    appendWorkspaceCondition(workspaceConditions, params, scope, "o.workspace");
