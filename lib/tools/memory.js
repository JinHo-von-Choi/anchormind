...
import { extractRequestCtx }    from "../memory/keyId.js";
import { exactScopeClause }      from "../memory/keyScope.js";
import { isSecurityPilotAutomationOff } from "../security-pilot.js";

function activityScope(args = {}) {
  return {
    keyId: args._keyId ?? null,
    groupKeyIds: Array.isArray(args._groupKeyIds) ? args._groupKeyIds : [],
    workspace: args.workspace ?? args._defaultWorkspace ?? null
  };
}
  ...
  try {
    ...
    if (args.includeContext) {
      ...
      const sessionResults = await Promise.all(
        sessionIds.map(sid => strictScope
          ? mgr.store.searchBySource(`session:${sid}`, agentId, keyId, 5, workspace, { strictScope: true })
          : mgr.store.searchBySource(`session:${sid}`, agentId, keyId, 5, workspace))
      );
...
export async function tool_forget(args) {
  ...
  return withAudit("forget", args, async (a, sessionId) => {
    ...
    SessionActivityTracker.record(sessionId, { tool: "forget", ...activityScope(a) }).catch(() => {});
...
export async function tool_link(args) {
  ...
  return withAudit("link", args, async (a, sessionId) => {
    ...
    SessionActivityTracker.record(sessionId, { tool: "link", ...activityScope(a) }).catch(() => {});
    return {
      response      : { success: true, ...result },
...
export async function tool_amend(args) {
  ...
  return withAudit("amend", args, async (a, sessionId) => {
    ...
    SessionActivityTracker.record(sessionId, { tool: "amend", fragmentId: a.id, ...activityScope(a) }).catch(() => {});
...
export async function tool_reflect(args) {
  ...
  return withAudit("reflect", args, async (a, sessionId) => {
    ...
    SessionActivityTracker.record(sessionId, { tool: "reflect", ...activityScope(a) }).catch(() => {});
    SessionActivityTracker.markReflected(sessionId, activityScope(a)).catch(() => {});
...
export async function tool_context(args) {
  ...
  try {
    ...
    SessionActivityTracker.record(sessionId, { tool: "context", ...activityScope(args) }).catch(() => {});
...
export async function tool_toolFeedback(args) {
  ...
  try {
    ...
    if (!isSecurityPilotAutomationOff()
        && process.env.ENABLE_RECONSOLIDATION === "true" && args.fragment_ids?.length > 0) {
      const action = args.relevant === false ? "decay" : "reinforce";
      const pool   = getPrimaryPool();
      if (pool) {
        const ownershipParams = [args.fragment_ids];
        const strictScope = args._keyId != null;
        const scope = {
          keyId: args._keyId ?? null,
          workspace: args.workspace ?? args._defaultWorkspace ?? null
        };
        const fromScope = strictScope ? exactScopeClause(ownershipParams, "from_frag", scope) : "";
        const toScope   = strictScope ? exactScopeClause(ownershipParams, "to_frag", scope) : "";
        pool.query(
          `SELECT fl.id FROM agent_memory.fragment_links fl
             JOIN agent_memory.fragments from_frag ON from_frag.id = fl.from_id
             JOIN agent_memory.fragments to_frag ON to_frag.id = fl.to_id
...
export async function tool_memoryStats(args) {
  ...
  const mgr = MemoryManager.getInstance();
  try {
    const { keyId }        = extractRequestCtx(args, { groupKeyIdsFallback: "empty" });
    const workspace        = args.workspace ?? args._defaultWorkspace ?? null;
    const strictScope      = keyId != null;
    const scope            = {
      keyId,
      workspace,
      strictScope,
      ...(strictScope ? {} : { groupKeyIds: args._groupKeyIds ?? (keyId ? [keyId] : []) })
    };
    const result          = await mgr.stats(scope);
    ...
    const scopedRequest = keyId != null;
    const neutralEvaluation = { precision_at_5: null, sample_sessions: 0, sufficient_rate: null };
    const neutralTaskSuccess = { success_rate: null, total_sessions: 0 };
    // Pilot feedback/search tables do not carry the full exact tuple yet; omit
    // these aggregates under scoped requests rather than returning cross-tenant data.
    const [evaluation, taskSuccess, irrelevance, searchObs] = scopedRequest
      ? [neutralEvaluation, neutralTaskSuccess, null, null]
      : await Promise.all([
          computeRollingPrecision(100).catch(() => neutralEvaluation),
          computeTaskSuccessRate(30).catch(() => neutralTaskSuccess),
...
export async function tool_memoryConsolidate(args, { onProgress = null } = {}) {
  ...
  delete args._sessionId;
  if (isSecurityPilotAutomationOff()) {
    return { success: false, disabled: true, error: "memory_consolidate is disabled in the security pilot" };
  }
  if (keyId != null) {
