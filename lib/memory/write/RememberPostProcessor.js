...
export class RememberPostProcessor {
   ...
   */
  async run(fragment, { agentId, keyId, groupKeyIds = null, workspace = fragment.workspace ?? null, strictScope = false }) {
    ...
    const morphemeP = this.morphemeIndex.tokenize(fragment.content)
      ...
      .then(() => this._updateMorphemeIndexed(id, keyId, workspace, strictScope))
      .catch(err => {
    ...
    if (fragment.linked_to?.length > 0) {
      ...
      try {
        const owned = await this.store.getByIds(linkIds, agentId, keyId, groupKeyIds, {
          workspace, strictScope
        });
        for (const f of owned) {
          if (strictScope && (f.key_id !== keyId || f.workspace !== workspace)) continue;
          allowedIds.add(f.id);
        }
      } catch (err) {
      ...
      await Promise.all([...allowedIds].map(async (linkId) => {
        ...
        await this._advisoryCycleCheck(id, linkId, "related", agentId, keyId, workspace, strictScope);
        return this.store.createLink(id, linkId, "related", agentId, 1, {
    ...
    this.conflictResolver
      ...
      .then(({ assertionStatus }) => {
        if (assertionStatus !== "observed") {
          this.store.patchAssertion(id, assertionStatus, keyId, workspace, { strictScope })
    ...
    this._proactiveRecallPromise = this._proactiveRecall(fragment, {
      agentId, keyId, workspace, strictScope
    }).catch(err => {
   ...
   */
  async _updateMorphemeIndexed(fragmentId, keyId = null, workspace = null, strictScope = false) {
    const pool = getPrimaryPool();
    if (!pool) return;
    try {
      const params = [fragmentId];
      const scope = strictScope ? exactScopeClause(params, "f", { keyId, workspace }) : "";
      await pool.query(
        `UPDATE ${SCHEMA}.fragments f SET morpheme_indexed = true WHERE f.id = $1${scope}`,
        params
      );
    } catch (err) {
   ...
   */
  async _proactiveRecall(fragment, { agentId, keyId, workspace = fragment.workspace ?? null, strictScope = false }) {
    ...
    for (const candidate of candidates) {
      ...
      if (strictScope && (candidate.key_id !== keyId || candidate.workspace !== workspace)) continue;
      ...
      if (mode === "legacy") {
        ...
        await this._advisoryCycleCheck(fragment.id, candidate.id, "related", agentId, keyId, workspace, strictScope);
        await this.store.createLink(fragment.id, candidate.id, "related", agentId, 1, {
          keyId, workspace, strictScope
        })
      ...
      if (SYMBOLIC_CONFIG.enabled && SYMBOLIC_CONFIG.proactiveGate) {
        const gateResult = await this._proactiveGateCheck(fragment, candidate, keyId, workspace, strictScope);
   ...
   */
  async _proactiveGateCheck(source, target, keyId, workspace = null, strictScope = keyId != null) {
    try {
      const detector       = this._getClaimConflictDetector();
      const proactiveConfig = MEMORY_CONFIG.proactiveRecall;
      return await evaluateProactiveGate({ source, target, keyId, workspace, strictScope }, { detector, proactiveConfig });
    } catch (err) {
   ...
   * @param {string|null}   keyId
   */
  async _advisoryCycleCheck(fromId, toId, relationType, agentId, keyId, workspace = null, strictScope = false) {
