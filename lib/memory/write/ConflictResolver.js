...
export class ConflictResolver {
   ...
   */
  async detectConflicts(content, topic, newId, agentId = "default", keyId = null, workspace = null, strictScope = false) {
    try {
      ...
      for (const frag of result.fragments) {
        if (strictScope && (frag.key_id !== keyId || frag.workspace !== workspace)) continue;
   ...
   */
  async autoLinkOnRemember(newFragment, agentId, workspace = newFragment.workspace ?? null, strictScope = false) {
    ...
    try {
      ...
      for (const frag of related) {
        ...
        if (strictScope && (frag.key_id !== newFragment.key_id || frag.workspace !== workspace)) continue;
        try {
          /** Phase 3 advisory cycle check — related는 non_directional이라 early return */
          await this._advisoryCycleCheck(newFragment.id, frag.id, "related", agentId, newFragment.key_id ?? null, workspace, strictScope);
          await this.store.createLink(newFragment.id, frag.id, "related", agentId, 1, {
            keyId: newFragment.key_id ?? null, workspace, strictScope
          });
          linkCount++;
   ...
   */
  async checkAssertionConsistency(fragment, agentId, keyId, workspace = fragment.workspace ?? null, strictScope = false) {
    ...
    try {
      ...
      const { rows } = await pool.query(
        `SELECT f.id, f.content, f.assertion_status
         FROM agent_memory.fragments f
         WHERE f.topic = $1
           AND f.created_at BETWEEN NOW() - INTERVAL '7 days' AND NOW() + INTERVAL '7 days'
           AND f.id != $2
           ${keyFilter}
      ...
      if (downgrade && supersedeCandidates.length > 0 && process.env.ENABLE_RECONSOLIDATION === "true") {
        for (const candidateId of supersedeCandidates) {
          quarantineAdjacentLinks(fragment.id, candidateId, keyId, workspace).catch(() => {});
        }
      }
      ...
      if (SYMBOLIC_CONFIG.enabled && SYMBOLIC_CONFIG.polarityConflict) {
        try {
          ...
          const polarity    = await detector.detectPolarityConflicts(fragment.id, keyId, workspace, strictScope);
   ...
   * @param {string} agentId
   */
  async supersede(oldId, newId, agentId = "default", keyId = null, workspace = null, strictScope = false) {
    /** Phase 3 advisory cycle check — superseded_by는 방향성 관계 */
    await this._advisoryCycleCheck(oldId, newId, "superseded_by", agentId, keyId, workspace, strictScope);
    await this.store.createLink(oldId, newId, "superseded_by", agentId, 1, {
