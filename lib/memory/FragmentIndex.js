...
export function workingMemoryRedisKey(sessionId, { keyId = null, workspace = null } = {}) {
  if (!sessionId) return null;
  /** Legacy sessions are available only when no authenticated key is present. */
  if (keyId == null) return `${WM_PREFIX}${sessionId}`;
  if (workspace == null || Array.isArray(keyId) || Array.isArray(workspace)) return null;
  const encode = value => Buffer.from(String(value)).toString("base64url");
  return `${WM_PREFIX}${sessionId}:k:${encode(keyId)}:w:${encode(workspace)}`;
}
...
export class FragmentIndex {
  ...
  /**
     ...
     */
  async addToWorkingMemory(sessionId, fragment, scope = {}) {
    if (!redisClient || redisClient.status !== "ready" || !sessionId) return;

    const key = workingMemoryRedisKey(sessionId, scope);
    if (!key) return;

    try {
      const entry = JSON.stringify({
        id              : fragment.id,
        key_id          : fragment.key_id ?? scope.keyId ?? null,
        workspace       : fragment.workspace ?? scope.workspace ?? null,
  ...
  /**
     ...
     */
  async getWorkingMemory(sessionId, scope = {}) {
    if (!redisClient || redisClient.status !== "ready" || !sessionId) return [];
    ...
    try {
      ...
      if (scope.keyId == null) return parsed;
      return parsed.filter(item => item.key_id === scope.keyId && item.workspace === scope.workspace);
    } catch (err) {
  ...
  /**
     ...
     */
  async clearWorkingMemory(sessionId, scope = {}) {
    if (!redisClient || redisClient.status !== "ready" || !sessionId) return;
    const key = workingMemoryRedisKey(sessionId, scope);
    if (key) await redisClient.del(key).catch(() => {});
  }
   ...
   */
  async evictWorkingMemoryItems(sessionId, ids, scope = {}) {
