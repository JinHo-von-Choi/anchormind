...
export async function reconsolidate(linkId, action, { triggeredBy = null, keyId = null, workspace = null } = {}) {
  if (isSecurityPilotAutomationOff()) return null;
  ...
  try {
    ...
    const updateParams = [delta, action, triggeredBy, linkId];
    const strictScope = keyId != null;
    const fromScope = strictScope
      ? exactScopeClause(updateParams, "from_frag", { keyId, workspace })
      : "";
    const toScope = strictScope
      ? exactScopeClause(updateParams, "to_frag", { keyId, workspace })
      : "";
    const res = await client.query(`
      UPDATE agent_memory.fragment_links fl
      ...
      WHERE fl.id = $4 AND fl.deleted_at IS NULL
        AND EXISTS (
          SELECT 1 FROM agent_memory.fragments from_frag
           WHERE from_frag.id = fl.from_id${fromScope}
        )
        AND EXISTS (
          SELECT 1 FROM agent_memory.fragments to_frag
           WHERE to_frag.id = fl.to_id${toScope}
        )
      RETURNING id,
...
export async function quarantineAdjacentLinks(fromId, toId, keyId, workspace = null) {
  if (isSecurityPilotAutomationOff()) return [];
  const pool  = getPrimaryPool();
  const params = [fromId, toId];
  const strictScope = keyId != null;
  const fromScope = strictScope ? exactScopeClause(params, "from_frag", { keyId, workspace }) : "";
  const toScope = strictScope ? exactScopeClause(params, "to_frag", { keyId, workspace }) : "";
  const links = await pool.query(`
    ...
    WHERE (fl.from_id = $1 AND fl.to_id = $2 OR fl.from_id = $2 AND fl.to_id = $1)
      AND fl.relation_type IN ('related', 'temporal')
      AND fl.deleted_at IS NULL
      AND (fl.quarantine_state IS NULL OR fl.quarantine_state = 'released')
      ${fromScope}
      ${toScope}
  `, params);
