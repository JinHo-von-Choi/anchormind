...
export class ContradictionDetector {
   ...
   */
  async detectContradictions(scope = {}) {
    ...
    const newFragParams = [];
    const newFragScope = exactScopeClause(newFragParams, "f", scope);
    let newFragsQuery = `
      SELECT f.id, f.content, f.topic, f.type, f.importance, f.embedding, f.created_at, f.key_id, f.workspace
      FROM ${SCHEMA}.fragments f
      WHERE f.embedding IS NOT NULL${newFragScope}`;

    const params = newFragParams;
    if (lastCheckAt) {
      const lastCheckIdx = params.length + 1;
      params.push(lastCheckAt);
      newFragsQuery += ` AND f.created_at > $${lastCheckIdx}`;
    }
    newFragsQuery += ` ORDER BY f.created_at DESC LIMIT 20`;
    ...
    for (const newFrag of newFrags.rows) {
      const candidateParams = [newFrag.id, newFrag.topic, newFrag.key_id ?? null];
      const candidateScope = exactScopeClause(candidateParams, "c", scope);
      ...
      for (const candidate of candidates.rows) {
        ...
        if (nliAvail) {
          ...
          if (nliResult) {
            if (nliResult.contradicts && !nliResult.needsEscalation) {
              const resolved = await this.resolveContradiction(newFrag, candidate,
                `NLI contradiction (conf=${nliResult.confidence.toFixed(3)})`, scope);
              if (resolved) {
                found++;
                nliResolved++;
              }
        ...
        try {
          ...
          if (verdict.contradicts) {
            const resolved = await this.resolveContradiction(newFrag, candidate, verdict.reasoning, scope);
            if (resolved) found++;
          }
          if (!latestProcessed || newFrag.created_at > latestProcessed) {
            latestProcessed = newFrag.created_at;
   ...
   */
  async resolveContradiction(newFrag, candidate, reasoning, scope = {}) {
    /** cross-tenant 보호: key_id가 다르면 처리 거부 */
    const nk = newFrag.key_id   ?? null;
    const ck = candidate.key_id ?? null;
    const nw = newFrag.workspace ?? null;
    const cw = candidate.workspace ?? null;
    if (nk !== ck || nw !== cw || (scope.workspace != null && (nw !== scope.workspace || cw !== scope.workspace))) {
      logWarn(`[ContradictionDetector] Cross-tenant resolveContradiction blocked: ` +
              `${newFrag.id}(key=${nk}) vs ${candidate.id}(key=${ck})`);
      return false;
    }

    if (scope.keyId != null && (nk !== scope.keyId || ck !== scope.keyId)) return false;
    await this.store.createLink(newFrag.id, candidate.id, "contradicts", "system", 1, {
      keyId: scope.keyId ?? nk,
      workspace: scope.workspace ?? nw,
      strictScope: scope.keyId != null || scope.workspace != null || nk != null || nw != null
    });
    ...
    if (newDate > oldDate) {
      if (!candidate.is_anchor) {
        const updateParams = [candidate.id];
        const updateScope = exactScopeClause(updateParams, "f", scope);
        await queryWithAgentVector("system",
          `UPDATE ${SCHEMA}.fragments f SET importance = importance * 0.5 WHERE f.id = $1${updateScope}`,
          updateParams, "write"
        );
      }
      await this.store.createLink(candidate.id, newFrag.id, "superseded_by", "system", 1, {
        keyId: scope.keyId ?? nk,
        workspace: scope.workspace ?? nw,
        strictScope: scope.keyId != null || scope.workspace != null || nk != null || nw != null
      });
      const validToParams = [candidate.id];
      const validToScope = exactScopeClause(validToParams, "f", scope);
      await queryWithAgentVector("system",
        `UPDATE ${SCHEMA}.fragments f SET valid_to = NOW()
         WHERE f.id = $1 AND f.valid_to IS NULL${validToScope}`,
        validToParams, "write"
      );
    } else {
      const importanceParams = [newFrag.id];
      const importanceScope = exactScopeClause(importanceParams, "f", scope);
      await queryWithAgentVector("system",
        `UPDATE ${SCHEMA}.fragments f SET importance = importance * 0.5 WHERE f.id = $1${importanceScope}`,
        importanceParams, "write"
      );
      await this.store.createLink(newFrag.id, candidate.id, "superseded_by", "system", 1, {
        keyId: scope.keyId ?? nk,
        workspace: scope.workspace ?? nw,
        strictScope: scope.keyId != null || scope.workspace != null || nk != null || nw != null
      });
      const validToParams = [newFrag.id];
      const validToScope = exactScopeClause(validToParams, "f", scope);
      await queryWithAgentVector("system",
        `UPDATE ${SCHEMA}.fragments f SET valid_to = NOW()
         WHERE f.id = $1 AND f.valid_to IS NULL${validToScope}`,
      ...
      try {
        ...
        if (verdict.contradicts) {
          const fragAResult = await queryWithAgentVector("system",
            `SELECT id, content, created_at, is_anchor, key_id, workspace FROM ${SCHEMA}.fragments WHERE id = $1`,
            [entry.idA]
          );
          ...
          if (fragAResult.rows.length && fragBResult.rows.length) {
            ...
            const fragA = fragAResult.rows[0];
            await this.resolveContradiction(fragA, fragBResult.rows[0], verdict.reasoning, {
              keyId: fragA.key_id ?? null,
              workspace: fragA.workspace ?? null
            });
            processed++;
          }
        }
