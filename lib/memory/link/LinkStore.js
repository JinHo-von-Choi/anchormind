...
export class LinkStore {
   ...
   */
  async createLink(fromId, toId, relationType = "related", agentId = "default", weight = 1, scope = {}) {
    /**
    ...
    const transactionResult = await withTransaction(getPrimaryPool(), async (client) => {
      ...
      if (scope?.strictScope === true) {
        const endpointParams = [fromId, toId];
        const endpointScope = exactScopeClause(endpointParams, "f", scope);
        const endpointResult = await client.query(
          `SELECT f.id FROM ${SCHEMA}.fragments f
            WHERE f.id = ANY($1::text[]) AND f.valid_to IS NULL${endpointScope}`,
          endpointParams
        );
        if (endpointResult.rows.length !== 2) return false;
      }
      await client.query(
    ...
    if (transactionResult === false) return false;

    /** 양방향 linked_to 갱신 */
    const fromParams = [fromId, toId];
    const fromScope = scope?.strictScope === true
      ? exactScopeClause(fromParams, "f", scope)
      : "";
    await queryWithAgentVector(agentId,
      `UPDATE ${SCHEMA}.fragments f
             SET linked_to = array_append(
                 CASE WHEN NOT ($2 = ANY(f.linked_to)) THEN f.linked_to ELSE f.linked_to END, $2
             )
             WHERE id = $1 AND NOT ($2 = ANY(linked_to))${fromScope}`,
      fromParams,
      "write"
    );
    const toParams = [fromId, toId];
    const toScope = scope?.strictScope === true
      ? exactScopeClause(toParams, "f", scope)
      : "";
    await queryWithAgentVector(agentId,
      `UPDATE ${SCHEMA}.fragments f
             SET linked_to = array_append(
                 CASE WHEN NOT ($1 = ANY(f.linked_to)) THEN f.linked_to ELSE f.linked_to END, $1
             )
             WHERE f.id = $2 AND NOT ($1 = ANY(f.linked_to))${toScope}`,
      toParams,
      "write"
    );
  }
  ...
  _buildLinkedFragmentsSql({
    ...
    if (!strictScope && workspace !== null) {
      params.push(workspace);
      workspaceFilter = strictScope
        ? `AND f.workspace = $${params.length}`
        : `AND (f.workspace = $${params.length} OR f.workspace IS NULL)`;
    }

    const sourceScope = strictScope
      ? exactScopeClause(params, "source", { keyId, workspace })
      : "";
    ...
    const sql = `SELECT DISTINCT ON (id)
         ...
         FROM (
           ...
           JOIN ${SCHEMA}.fragments source ON l.from_id = source.id
           ...
           SELECT f.id, f.content, f.topic, f.keywords, f.type,
                  ...
                  END AS relation_order
           FROM ${SCHEMA}.fragment_links l
           JOIN ${SCHEMA}.fragments source ON l.to_id = source.id
  ...
  async getRCAChain(startId, agentId = "default", keyId = null, groupKeyIds = [], opts = {}) {
    ...
    const seedKeyFilter = strictScope
      ? exactScopeClause(params, "f", { keyId, workspace: opts.workspace ?? null })
      : keyScopeClause(params, "f.key_id", scope);
    const keyFilter = strictScope
      ? exactScopeClause(params, "f2", { keyId, workspace: opts.workspace ?? null })
      : keyScopeClause(params, "f2.key_id", scope);
