...
export class SessionLinker {
  ...
  async autoLinkSessionFragments(fragments, agentId = "default", keyId = null, groupKeyIds = [], workspace = null) {
    const legacyFragments = Array.isArray(fragments)
      && fragments.every(fragment => fragment?.key_id == null
        && fragment?.workspace == null
        && fragment?.group_key_ids == null);
    if (keyId != null
    ...
    const scopedFragments = workspace != null
      ? fragments.filter(fragment => fragment.key_id === keyId
          ...
          && (groupKeyIds.length === 0 || groupKeyIds.includes(fragment.key_id)))
      : fragments;
