...
export class MemoryRecaller {
  ...
  async recall(params) {
    ...
    // Pilot strict mode is activated by an explicit API key; workspace-only
    // calls retain the legacy workspace behavior for compatibility.
    ...
    if (!isSecurityPilotAutomationOff()
        && params.contextText && process.env.ENABLE_SPREADING_ACTIVATION === "true") {
    ...
    const result = await this.search.search({
      ...
      agentId,                                // RLS 컨텍스트
      keyId: strictScope ? keyId : groupKeyIds, // strict pilot: scalar exact key; legacy: group array
      workspace,                              // 워크스페이스 필터
      strictScope,
    ...
    const rankCtx = { lexicalQuery, anchorTime, config: MEMORY_CONFIG, workspace };
    ...
    if (result.count === 0 && params.topic) {
      result._topicCandidates = await suggestTopics(
        this.store,
        { keyId, groupKeyIds, workspace, strictScope },
        params.topic
      );
    }
    ...
    if (params.sessionId && result.fragments && result.fragments.length >= 2) {
      ...
      new GraphLinker()
        .buildCoRetrievalLinks(fragIds, params.sessionId, agentId, { keyId, workspace })
  ...
  async context(params) {
    ...
    /** API 키가 있으면 ContextBuilder가 scalar key + workspace strict 경로를 선택한다. */
    if (params._keyId != null) contextParams._keyId = params._keyId;
    const result = await this.contextBuilder.build(contextParams);
  ...
  async toolFeedback(params) {
    ...
    if (fragmentIds && fragmentIds.length > 0) {
      try {
        ...
        if (keyId != null || workspace != null) {
          keyFilter = exactScopeClause(emaParams, "fragments", { keyId, workspace });
        }
        await pool.query(
          `UPDATE agent_memory.fragments AS fragments
  ...
  async fragmentHistory(params) {
    ...
    const historyOpts = { includePeerAgents: params.includePeerAgents === true };
    if (params._keyId != null) historyOpts.strictScope = true;
    if (workspace != null) historyOpts.workspace = workspace;
  ...
  async graphExplore(params) {
    ...
    const strictScope = params._keyId != null;
    const startFrag = await this.store.getById(params.startId, agentId, strictScope ? keyId : keyId, groupKeyIds, {
      workspace,
      strictScope
    });
    if (!startFrag) {
