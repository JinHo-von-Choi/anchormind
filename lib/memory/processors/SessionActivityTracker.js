...
export class SessionActivityTracker {
  ...
  static async record(sessionId, data) {
    ...
    try {
      ...
      /** Scope metadata is part of the activity record, never inferred later. */
      if (data.keyId !== undefined || data._keyId !== undefined) {
        log.keyId = data.keyId ?? data._keyId ?? null;
      }
      if (data.groupKeyIds !== undefined || data._groupKeyIds !== undefined) {
        const groups = data.groupKeyIds ?? data._groupKeyIds;
        log.groupKeyIds = Array.isArray(groups) ? [...new Set(groups)] : [];
      }
      if (data.workspace !== undefined) {
        log.workspace = data.workspace ?? null;
      }
   ...
   */
  static async getActivity(sessionId, scope = {}) {
    if (!sessionId || !redisClient || redisClient.status !== "ready") return null;

    try {
      const activity = await this._getLog(`${KEY_PREFIX}${sessionId}`);
      return this._matchesScope(activity, scope) ? activity : null;
    } catch {
      return null;
    }
  }
   ...
   * @param {string} sessionId
   */
  static async markReflected(sessionId, scope = {}) {
    await this.record(sessionId, { action: "reflected", ...scope });
  }

  static _matchesScope(activity, scope = {}) {
    const keyId = scope.keyId ?? scope._keyId ?? null;
    const workspace = scope.workspace ?? null;
    const groupKeyIds = Array.isArray(scope.groupKeyIds)
      ? scope.groupKeyIds
      : (Array.isArray(scope._groupKeyIds) ? scope._groupKeyIds : []);
    const strict = keyId != null || workspace != null || groupKeyIds.length > 0;
    if (!strict) return true;
    if (keyId != null && (activity.keyId ?? activity.key_id) !== keyId) return false;
    if (workspace != null && activity.workspace !== workspace) return false;
    if (groupKeyIds.length > 0) {
      const actual = Array.isArray(activity.groupKeyIds) ? [...new Set(activity.groupKeyIds)].sort() : [];
      const expected = [...new Set(groupKeyIds)].sort();
