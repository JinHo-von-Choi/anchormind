...
export class MemoryRememberer {
  ...
  async remember(params) {
    ...
    params = await this._autoAssignCaseId(params, sessionId, keyId, groupKeyIds, workspace);
   ...
   */
  async _autoAssignCaseId(params, sessionId, keyId, groupKeyIds, workspace = null) {
    ...
    const strictScope = keyId != null;
    const scopeOpts = { workspace, strictScope };
    const existingCaseId = await this.store.findCaseIdBySessionTopic(
      sessionId, params.topic, keyId, groupKeyIds, workspace, scopeOpts
    );
    if (existingCaseId) {
    ...
    if (errorIds.length > 0) {
      ...
      Promise.all(errorIds.map(id => this.store.updateCaseId(id, newCaseId, keyId, workspace, scopeOpts)))
  ...
  async _persistNonAtomic(fragment, params, agentId, keyId, groupKeyIds) {
    ...
    const workspace = fragment.workspace ?? params.workspace ?? params._defaultWorkspace ?? null;
    const strictScope = keyId != null;
    await this.postProcessor.run(
      { ...fragment, id },
      { agentId, keyId, groupKeyIds, workspace, strictScope }
    );
    ...
    const conflicts = params.skipConflictDetection
      ...
      : await this.conflictResolver.detectConflicts(
          fragment.content, fragment.topic, id, agentId, keyId, workspace, strictScope
        );

    /** 자동 링크 생성 (유사 파편 기반) */
    await this.conflictResolver.autoLinkOnRemember({ ...fragment, id }, agentId, workspace, strictScope).catch(err => {
    ...
    if (params.supersedes && Array.isArray(params.supersedes)) {
      for (const oldId of params.supersedes) {
        ...
        try {
          await this._supersede(oldId, id, agentId, keyId, workspace);
        } catch (err) {
    ...
    if (effectiveImportance < 0.3) {
      ...
      if (!params.ttl_tier) {
        ...
        await this.store.updateTtlTier(id, "short", keyId, workspace, { strictScope }).catch(err => {
    ...
    if (fragment.case_id && this.caseEventStore) {
      this._recordCaseEvent({ ...fragment, id }, keyId, workspace, strictScope).catch(err =>
  ...
  async _runPolicyGate(fragment, { keyId, mode }) {
    ...
    try {
      ...
      /** ApiKeyStore 조회 실패 시 fail-open: 경고 없이 통과 */
      logWarn(`[MemoryRememberer] workspace allow-list check failed (fail-open): ${err.message}`);
    }
    ...
    /** hard gate 대상 필터: workspaceNotAllowed는 저장 거부 대상이 아니므로 항상 제외하고,
     *  fragmentHasWorkspace는 MEMENTO_WORKSPACE_GATE=true일 때만 포함한다. */
    const workspaceGateEnabled = process.env.MEMENTO_WORKSPACE_GATE === "true";
    const gateEligible = violations.filter(v => {
      if (v.rule === "workspaceNotAllowed")  return false;
  ...
  async _finalizeRemember(fragment, { agentId, keyId, groupKeyIds, params }) {
    ...
    const strictScope = keyId != null;
    await this.postProcessor.run(fragment, { agentId, keyId, groupKeyIds, workspace, strictScope });
    ...
    await this.conflictResolver.autoLinkOnRemember(fragment, agentId, workspace, strictScope).catch(err => {
      logWarn(`[MemoryRememberer] autoLinkOnRemember failed: ${err.message}`);
    });
    ...
    if (fragment.case_id && this.caseEventStore) {
      this._recordCaseEvent(fragment, keyId, workspace, strictScope).catch(err =>
        logWarn(`[MemoryRememberer] case event recording failed: ${err.message}`)
      );
    }
   ...
   */
  async _recordCaseEvent(fragment, keyId, workspace = fragment.workspace ?? null, strictScope = false) {
    ...
    await this.caseEventStore.addEvidence(fragment.id, event_id, "produced_by", 1, {
      keyId, workspace, strictScope
    }).catch(() => {});

    /** preceded_by: 동일 case_id의 직전 이벤트와 연결 */
    const prevEvents = await this.caseEventStore.getByCase(fragment.case_id, {
      limit: 2, keyId, workspace, strictScope
    });
    const prevEvent  = prevEvents.find(e => e.event_id !== event_id);
    if (prevEvent) {
      await this.caseEventStore.addEdge(event_id, prevEvent.event_id, "preceded_by", 1, {
        keyId, workspace, strictScope
      }).catch(() => {});
    }
    ...
    if (eventType === "fix_attempted") {
      const errorEvents = await this.caseEventStore.getByCase(
        fragment.case_id,
        { eventType: "error_observed", keyId, workspace, strictScope }
      );
      for (const errEvt of errorEvents) {
        await this.caseEventStore.addEdge(event_id, errEvt.event_id, "resolved_by", 1, {
          keyId, workspace, strictScope
        }).catch(() => {});
      }
    }
   ...
   * @param {string|null} keyId
   */
  async _supersede(oldId, newId, agentId = "default", keyId = null, workspace = null) {
    return this.conflictResolver.supersede(oldId, newId, agentId, keyId, workspace, keyId != null);
  }
  ...
  async amend(params) {
    ...
    const workspace   = params.workspace ?? params._defaultWorkspace ?? null;
    const strictScope = params._keyId != null;
    const scopeOpts   = { workspace, strictScope };
    const existing    = await this.store.getById(params.id, agentId, keyId, groupKeyIds, scopeOpts);
    if (!existing) {
      /** 만료 파편과 미존재·권한 부족을 구분한다. 만료 파편은 이력이므로 수정 대상이 아니다. */
      const expired = await this.store.getById(
        params.id, agentId, keyId, groupKeyIds, { includeExpired: true, ...scopeOpts }
      ).catch(() => null);
    ...
    if (
      ...
      if (amendEventType) {
        this.caseEventStore.append({
          ...
          workspace,
          strictScope
        }, { keyId, workspace, strictScope }).then(({ event_id }) =>
          this.caseEventStore.addEvidence(existing.id, event_id, "produced_by", 1, {
            keyId, workspace, strictScope
          }).catch(() => {})
  ...
  async forget(params) {
    ...
    if (params.dryRun === true && params.id) {
      const frag = await this.store.getById(params.id, agentId, keyId, groupKeyIds, scopeOpts);
    ...
    if (params.id) {
      ...
      const ok = await this.store.delete(frag.id, agentId, keyId, workspace, scopeOpts);
      deleted  = ok ? 1 : 0;
    }
    ...
    if (params.topic) {
      const topicFrags = await this.store.searchByTopic(params.topic, {
        agentId,
        keyId: strictScope ? keyId : (groupKeyIds ?? (keyId ? [keyId] : undefined)),
        workspace,
        strictScope,
      ...
      for (const frag of topicFrags) {
        /** legacy는 그룹 소유권을 유지하지만 strict pilot은 SQL의 exact tuple만 허용한다. */
        if (!strictScope && keyId && frag.key_id !== keyId && (!groupKeyIds || !groupKeyIds.includes(frag.key_id))) {
          protected_++;
          continue;
        }
