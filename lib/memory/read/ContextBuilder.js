...
function buildRankedInjection(
  ...
  const sorted = otherFragments.filter(f => !isGuaranteed(f)).sort((a, b) => score(b) - score(a));
...
export class ContextBuilder {
  ...
  async build(params) {
    ...
    const { coreFragments, guaranteedCoreFragments } =
   ...
   *
   * @returns {{ typeFragMap: Map, types: string[], coreFragments: object[], guaranteedCoreFragments: object[], usedChars: number }}
   */
  async #loadCoreMemory(params, agentId, keyId, groupKeyIds, workspace, allWorkspaces) {
    ...
    const coreFragments = [];
    const guaranteedCoreFragments = [];
    for (const type of types) {
      const fragments = guaranteed.get(type) || [];
      coreFragments.push(...fragments);
      if (fragments.length > 0) guaranteedCoreFragments.push(fragments[0]);
    }

    return { typeFragMap, types, coreFragments, guaranteedCoreFragments, usedChars };
  }
