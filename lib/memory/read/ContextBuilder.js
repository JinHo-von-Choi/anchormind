 ...
 * ID가 없는 임시 항목은 서로 같은 파편인지 판별할 수 없으므로 원래 섹션에 보존한다.
 *
...
export class ContextBuilder {
  ...
  async build(params) {
    ...
    const { wmFragments } = await this.#loadWorkingMemory(params);

    const anchorFragments = await this.#loadAnchorMemory(groupKeyIds, workspace);
    const learningFragments = await this.#loadLearningFragments(agentId, keyId, workspace);
    ...
    let effective = deduped;
    let ranked    = null;

    /** structured 응답은 rankedInjection의 토큰 선택을 모든 표현에 동일하게 적용한다. */
    if (params.structured === true) {
      const others = [...deduped.core, ...deduped.learning, ...deduped.working];
      ranked = buildRankedInjection(
        deduped.anchor,
        others,
      ...
      const selectedIds = new Set(ranked.items.map(item => item.id));
      const retainSelected = fragment => !fragment?.id || selectedIds.has(fragment.id);
      effective = {
        anchor  : deduped.anchor.filter(retainSelected),
        core    : deduped.core.filter(retainSelected),
        learning: deduped.learning.filter(retainSelected),
        working : deduped.working.filter(retainSelected)
      };
    }
   ...
   */
  async #loadLearningFragments(agentId, keyId, workspace) {
    try {
