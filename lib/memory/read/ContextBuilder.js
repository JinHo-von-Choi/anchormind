...
export class ContextBuilder {
  ...
  async build(params) {
    ...
    const anchorFragments = await this.#loadAnchorMemory(keyId, groupKeyIds, workspace, strictScope);

    await this.#loadLearningFragments(typeFragMap, types, agentId, keyId, workspace, strictScope);
  ...
  async #loadCoreMemory(params, agentId, keyId, groupKeyIds, workspace) {
    ...
    function _tempScore(frag) {
      ...
      return score * computeWorkspaceDecayFactor(frag, workspace);
    }
   ...
   * @returns {object[]}
   */
  async #loadAnchorMemory(keyId, groupKeyIds, workspace, strictScope = false) {
    let anchorFragments = [];
    try {
      ...
      if (pool) {
        ...
        if (strictScope) {
          const scope = exactScopeClause(anchorParams, "f", { keyId, workspace });
          anchorKeyFilter = scope;
        } else {
          if (groupKeyIds != null) {
            anchorParams.push(groupKeyIds);
            anchorKeyFilter = `AND f.key_id = ANY($${anchorParams.length})`;
          }
          if (workspace != null) {
        ...
        const anchorResult = await pool.query(
          `SELECT f.id, f.content, f.type, f.topic, f.importance, f.key_id, f.workspace
        ...
        anchorFragments = strictScope
          ? anchorResult.rows.filter(row => row.key_id === keyId && row.workspace === workspace)
          : anchorResult.rows;
      }
    } catch (err) {
   ...
   */
  async #loadLearningFragments(typeFragMap, types, agentId, keyId, workspace, strictScope = false) {
    try {
      const args = ["learning_extraction", agentId, keyId, 5, workspace];
      if (strictScope) args.push({ strictScope: true });
      const learningFrags = await this.#store.searchBySource(...args);
