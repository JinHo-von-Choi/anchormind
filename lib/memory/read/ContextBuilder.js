...
import { byDescendingScore, compareTimestampDescThenId, deterministicOrderBy } from "./DeterministicRanking.js";
...
export function selectAnchorFragments(workspaceCandidates, globalCandidates, {
  ...
  const candidates = {
    workspace: workspaceApplied ? workspaceCandidateCount : 0,
    global   : globalCandidateCount,
  };
  candidates.total = candidates.workspace + candidates.global;
  ...
  const excluded = {
    workspace: Math.max(0, candidates.workspace - selectedCounts.workspace),
    global   : Math.max(0, candidates.global - selectedCounts.global),
  };
  excluded.total = excluded.workspace + excluded.global;
...
export class ContextBuilder {
  ...
  async build(params) {
    ...
    const { fragments: anchorFragments, meta: anchorSelection } =
      await this.#loadAnchorMemory(groupKeyIds, workspace);
    ...
    if (params.structured === true) {
      return this.#buildStructuredResponse({
        ...
        dedupResult, anchorTokens, coreTokens, wmTokens, workspace, anchorSelection
      });
    }
  ...
  async #loadAnchorMemory(groupKeyIds, workspace) {
    ...
    const empty = () => selectAnchorFragments([], [], {
      limit: maxAnchors, reserve, workspaceApplied: workspace != null,
    });

    try {
      const pool = this.#getPool();
      if (pool) {
        const queryCandidates = async ({ workspaceValue, globalOnly }) => {
          const anchorParams    = [];
          ...
          const result = await pool.query(
            `SELECT id, content, type, topic, importance, workspace, created_at,
              ...
              ORDER BY importance DESC, created_at DESC NULLS LAST, id ASC
        ...
        if (workspace == null) {
          const global = await queryCandidates({ globalOnly: true });
          return selectAnchorFragments([], global.fragments, {
            limit: maxAnchors,
            reserve,
            workspaceApplied: false,
            workspaceCandidateCount: 0,
            globalCandidateCount: global.candidateCount,
          });
        }
        const [workspaceResult, globalResult] = await Promise.all([
          queryCandidates({ workspaceValue: workspace, globalOnly: false }),
          queryCandidates({ globalOnly: true }),
        ]);
        return selectAnchorFragments(workspaceResult.fragments, globalResult.fragments, {
          limit: maxAnchors,
          reserve,
