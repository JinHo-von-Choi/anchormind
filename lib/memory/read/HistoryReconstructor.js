...
export class HistoryReconstructor {
  ...
  async reconstruct(params) {
    ...
    const timeline = await this._fetchTimelineParameterized({
      ...
      allWorkspaces,
      agentId: agentScope.agentId, includePeerAgents: agentScope.includePeerAgents
    });
  ...
  async _fetchTimelineParameterized({
    ...
    workspace, allWorkspaces, agentId, includePeerAgents
  }) {
    const pool   = getPrimaryPool();
    const params = [];
    const agentScope = resolveAgentScope({ agentId, includePeerAgents });
