...
export class HistoryReconstructor {
  ...
  async reconstruct(params) {
    ...
    if (this.caseEventStore && caseId) {
      case_events = await this.caseEventStore.getByCase(caseId, { keyId, groupKeyIds, workspace }).catch(err => {
      ...
      if (case_events.length > 0) {
        ...
        event_dag      = await this.caseEventStore.getEdgesByEvents(eventIds, { keyId, groupKeyIds, workspace }).catch(() => []);
      }
    }
    ...
    if (case_events.length > 0 && this.caseEventStore) {
      await Promise.all(case_events.map(async (evt) => {
        evt.evidence = await this.caseEventStore.getEvidenceByEvent(evt.event_id, { keyId, groupKeyIds, workspace }).catch(() => []);
      }));
    }
  ...
  async _fetchTimelineParameterized({ caseId, entity, timeRange, query, limit, keyId, groupKeyIds, workspace }) {
    ...
    const keyClause = (keyId != null || workspace != null)
      ? exactScopeClause(params, "f", { keyId, workspace })
      : keyScopeClause(params, "f.key_id", { keyId, groupKeyIds });

    /** workspace 격리 */
    let wsClause = "";
    /** exactScopeClause already binds workspace without NULL/global expansion. */
  ...
  async _fetchLinks(fragmentIds, scope = {}) {
    ...
    const fromScope = exactScopeClause(params, "from_frag", scope);
    const toScope = exactScopeClause(params, "to_frag", scope);
