...
export class HistoryReconstructor {
  ...
  async _fetchLinks(fragmentIds, scope = {}) {
    ...
    const fromScope = scope.strictScope
      ? exactScopeClause(params, "from_frag", scope)
      : "";
    const toScope = scope.strictScope
      ? exactScopeClause(params, "to_frag", scope)
      : "";
    const { rows }     = await pool.query(
      `SELECT fl.from_id, fl.to_id, fl.relation_type, fl.weight
        ...
        WHERE (fl.from_id = ANY($1)${fromScope}
           OR fl.to_id   = ANY($1)${toScope})`,
      params
    );
    return rows;
  }
