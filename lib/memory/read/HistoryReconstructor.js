...
export class HistoryReconstructor {
  ...
  async reconstruct(params) {
    ...
    const scope       = { keyId, workspace, strictScope };

    if (!caseId && !entity) {
      throw new Error("caseId 또는 entity 중 하나는 필수입니다.");
    }

    const timeline = await this._fetchTimelineParameterized({ caseId, entity, timeRange, query, limit, keyId, groupKeyIds, workspace, strictScope });
    ...
    if (this.caseEventStore && caseId) {
      case_events = await this.caseEventStore.getByCase(caseId, { keyId, groupKeyIds, workspace, strictScope }).catch(err => {
      ...
      if (case_events.length > 0) {
        ...
        event_dag      = await this.caseEventStore.getEdgesByEvents(eventIds, { keyId, groupKeyIds, workspace, strictScope }).catch(() => []);
      }
    }
    ...
    if (case_events.length > 0 && this.caseEventStore) {
      await Promise.all(case_events.map(async (evt) => {
        evt.evidence = await this.caseEventStore.getEvidenceByEvent(evt.event_id, { keyId, groupKeyIds, workspace, strictScope }).catch(() => []);
      }));
    }
    ...
    const links       = await this._fetchLinks(fragmentIds, scope);
   ...
   * @private
   */
  async _fetchTimelineParameterized({ caseId, entity, timeRange, query, limit, keyId, groupKeyIds, workspace, strictScope = false }) {
    ...
    let keyClause = "";
    if (strictScope) {
      keyClause = exactScopeClause(params, "f", { keyId, workspace });
    } else {
      keyClause = keyScopeClause(params, "f.key_id", { keyId, groupKeyIds });
      if (workspace != null) {
   ...
   */
  async _fetchLinks(fragmentIds, scope = {}) {
    ...
    let fromScope;
    let toScope;
    if (scope.strictScope) {
      fromScope = exactScopeClause(params, "from_frag", scope);
      toScope = exactScopeClause(params, "to_frag", scope);
    } else {
      fromScope = keyScopeClause(params, "from_frag.key_id", scope);
      toScope = keyScopeClause(params, "to_frag.key_id", scope);
      if (scope.workspace != null) {
        params.push(scope.workspace);
        fromScope += ` AND (from_frag.workspace = $${params.length} OR from_frag.workspace IS NULL)`;
        params.push(scope.workspace);
        toScope += ` AND (to_frag.workspace = $${params.length} OR to_frag.workspace IS NULL)`;
      }
    }
    const { rows }     = await pool.query(
      `SELECT fl.from_id, fl.to_id, fl.relation_type, fl.weight
         FROM ${SCHEMA}.fragment_links fl
        JOIN ${SCHEMA}.fragments from_frag ON from_frag.id = fl.from_id
        JOIN ${SCHEMA}.fragments to_frag ON to_frag.id = fl.to_id
        WHERE fl.from_id = ANY($1)
          AND fl.to_id   = ANY($1)${fromScope}${toScope}`,
      params
    );
    /** DB predicates are authoritative, but reject hostile adapter rows too. */
    const timelineIds = new Set(fragmentIds);
    return rows.filter(row => timelineIds.has(row?.from_id) && timelineIds.has(row?.to_id));
  }
