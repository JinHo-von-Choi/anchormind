...
export class CaseRecall {
  ...
  async buildCaseTriples(
    fragments,
    { keyId = null, maxCases = 5, isAnchor, includeSuperseded = false } = {}
  ) {
     ...
     * 독립적인 이력으로 유지하고 case_events 자체의 key scope만 적용한다.
     */
    let events = [];
    try {
      const eventParams = [topCaseIds];
      const eventKeyFilter = keyScopeScalar(eventParams, "ce.key_id", keyId).trimStart();
