...
export class CaseRecall {
   ...
   * @param {boolean|undefined} opts.isAnchor - true=앵커만, false=비앵커만, 미지정=혼합
   * @returns {Promise<Object[]>} cases 배열
   *   [{ case_id, goal, outcome, resolution_status, events, fragment_count, relevance_score }]
   */
  async buildCaseTriples(fragments, { keyId = null, maxCases = 5, isAnchor } = {}) {
    ...
    try {
      const { rows } = await pool.query(
        `SELECT case_id, goal, outcome, resolution_status, phase, is_anchor,
      ...
      caseFrags = isAnchor === undefined
        ? rows
        : rows.filter(fragment => fragment.is_anchor === isAnchor);
    } catch (err) {
    ...
    try {
      ...
      let eventParams;
      if (isAnchor === undefined) {
        eventQuery = `SELECT case_id, event_type, summary, created_at
           FROM ${SCHEMA}.case_events
          WHERE case_id = ANY($1)
          ORDER BY case_id, sequence_no ASC`;
        eventParams = [topCaseIds];
      } else {
        eventParams = [topCaseIds, isAnchor];
        const eventKeyFilter = keyScopeScalar(eventParams, "f.key_id", keyId).trimStart();
        eventQuery = `SELECT ce.case_id, ce.event_type, ce.summary, ce.created_at,
                f.is_anchor AS source_is_anchor
      ...
      events = rows;
      if (isAnchor !== undefined) {
        events = events.filter(event => event.source_is_anchor === isAnchor);
      }
    } catch (err) {
