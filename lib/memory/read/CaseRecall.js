...
export class CaseRecall {
  ...
  async buildCaseTriples(fragments, {
    ...
    const keyFilter = keyScopeGroup(
      queryParams, "key_id", groupKeyIds ?? (keyId ? [keyId] : null)
    ).trimStart();
    const workspaceConditions = [];
    appendWorkspaceCondition(
      workspaceConditions, queryParams, { workspace, allWorkspaces }, "workspace"
    );
    ...
    if (!includePeerAgents) {
      ...
      agentFilter = `AND (agent_id = $${queryParams.length} OR agent_id = 'default')`;
    }

    let caseFrags;
    try {
      const { rows } = await pool.query(
        `SELECT id, case_id, goal, outcome, resolution_status, phase, created_at,
    ...
    try {
      ...
      const eventKeyFilter = keyScopeGroup(
        eventParams, "e.key_id", groupKeyIds ?? (keyId ? [keyId] : null)
      );
      const eventWorkspaceConditions = [];
      appendWorkspaceCondition(
        eventWorkspaceConditions, eventParams, { workspace, allWorkspaces }, "f.workspace"
      );
      const eventWorkspaceFilter = eventWorkspaceConditions.length > 0
        ? ` AND ${eventWorkspaceConditions[0]}`
        : "";
      let eventAgentFilter = "";
      if (!includePeerAgents) {
        eventParams.push(agentId);
        eventAgentFilter = ` AND (f.agent_id = $${eventParams.length} OR f.agent_id = 'default')`;
      }
      const sourceJoin = (allWorkspaces && includePeerAgents)
        ? ""
        : `JOIN ${SCHEMA}.fragments f
             ON f.id = e.source_fragment_id
            AND f.valid_to IS NULL`;
      const { rows } = await pool.query(
        `SELECT e.case_id, e.event_type, e.summary, e.created_at
           FROM ${SCHEMA}.case_events e
           ${sourceJoin}
          WHERE e.case_id = ANY($1)
            ${eventKeyFilter}
            ${eventWorkspaceFilter}
            ${eventAgentFilter}
          ORDER BY e.case_id, e.sequence_no ASC`,
        eventParams
      );
      events = rows;
    } catch (err) {
