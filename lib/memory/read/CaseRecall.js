...
export class CaseRecall {
  ...
  async buildCaseTriples(fragments, {
    ...
    let events = [];
    try {
      /** 이벤트 자체의 key를 다시 검증하고 nullable source 이력은 보존한다. */
      const eventParams = [topCaseIds];
      const eventKeyIds = groupKeyIds ?? (keyId ? [keyId] : null);
      /**
       * 비master는 현재 키 그룹 이벤트만 허용한다. case_id는 호출자가 지정할 수
       * 있으므로 NULL 레거시/master 이벤트까지 함께 허용하면 같은 case_id 충돌로
       * 다른 테넌트의 이벤트가 노출될 수 있다. master(null)만 조건 없이 전체를 본다.
       */
      const eventKeyFilter = keyScopeGroup(eventParams, "e.key_id", eventKeyIds)
        .trimStart();
      const { rows } = await pool.query(
