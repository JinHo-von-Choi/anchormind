...
export async function searchSyntheticQueries(vec, opts = {}) {
  ...
  appendWorkspaceCondition(workspaceConditions, params, opts, "f.workspace");
  const workspaceClause = workspaceConditions.length > 0
    ...
    const { rows } = await pool.query(
      `SELECT q.fragment_id, 1 - (q.embedding <=> $1::vector) AS similarity
        ...
        WHERE q.embedding IS NOT NULL
          ...
          AND 1 - (q.embedding <=> $1::vector) >= $2${keyClause}${workspaceClause}
        ORDER BY q.embedding <=> $1::vector ASC
        LIMIT $3`,
      params
    );
    hits = rows;
  ...
  const fragWorkspaceConditions = [];
  appendWorkspaceCondition(fragWorkspaceConditions, fragParams, opts, "f.workspace");
  const fragWorkspaceClause = fragWorkspaceConditions.length > 0
    ? ` AND ${fragWorkspaceConditions[0]}`
    : "";
    ...
    const { rows } = await pool.query(
      `SELECT f.*
        ...
        WHERE f.id = ANY($1::text[])
          AND f.valid_to IS NULL${fragKeyClause}${fragWorkspaceClause}`,
      fragParams
    );
