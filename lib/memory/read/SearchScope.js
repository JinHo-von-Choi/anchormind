...
export class SearchScope {
  ...
  constructor({
    ...
    this.strictScope       = strictScope === true;
  }
  ...
  applyTo(fragment) {
    ...
    if (this.strictScope) {
      /** Pilot API-key scope is an exact scalar key/workspace tuple. */
      if (Array.isArray(this.keyId) || this.keyId == null || this.workspace == null) return false;
      if (fragment.key_id !== this.keyId || fragment.workspace !== this.workspace) return false;
