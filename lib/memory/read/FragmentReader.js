...
export class FragmentReader {
   ...
   * @returns {string}
   */
  #agentScopeCond(paramRef, agentId, includePeerAgents, col = "agent_id") {
    return agentScopeCondition(paramRef, resolveAgentScope({
      agentId, includePeerAgents
    }), col);
  }

  async getById(id, agentId = "default", keyId = null, groupKeyIds = [], opts = {}) {
    const agentCond = this.#agentScopeCond("$2", agentId, opts.includePeerAgents === true);
  ...
  async getByIds(ids, agentId = "default", keyId = null, groupKeyIds = [], opts = {}) {
    ...
    const baseConds = ["id = ANY($1)", this.#agentScopeCond("$2", agentId, opts.includePeerAgents === true)];
  ...
  async getHistory(fragmentId, agentId = "default", keyId = null, groupKeyIds = [], opts = {}) {
    ...
    const versionScope = resolveAgentScope({
      agentId, includePeerAgents: opts.includePeerAgents
    });
    ...
    const chainAgentCond = this.#agentScopeCond("$2", agentId, opts.includePeerAgents === true, "f.agent_id");
  ...
  async searchByKeywords(keywords, options = {}) {
    ...
    const conditions = ["keywords && $1", this.#agentScopeCond("$2", agentId, options.includePeerAgents === true)];
  ...
  async searchByTopic(topic, options = {}) {
    const agentId    = options.agentId || "default";
    const isAnchor   = normalizeIsAnchor(options.isAnchor);
    const conditions = ["topic = $1", this.#agentScopeCond("$2", agentId, options.includePeerAgents === true)];
  ...
  async searchBySemantic(queryEmbedding, opts = {}) {
    ...
    const conditions = [
      ...
      this.#agentScopeCond("$4", agentId, includePeerAgents === true, "f.agent_id")
    ];
  ...
  async searchByTimeRange(from, to, options = {}) {
    const agentId    = options.agentId || "default";
    const isAnchor   = normalizeIsAnchor(options.isAnchor);
    const conditions = [this.#agentScopeCond("$1", agentId, options.includePeerAgents === true)];
  ...
  async searchBySource(
    ...
    let query = `SELECT id, content, topic, type, keywords, importance, source,
                        ...
                        case_id, goal, outcome, phase, resolution_status, assertion_status
                 FROM ${SCHEMA}.fragments
                 WHERE source = $1 AND ${this.#agentScopeCond("$2", effectiveAgentId, opts.includePeerAgents === true)}`;
  ...
  async searchAsOf(asOf, agentId = "default", opts = {}, keyId = null) {
    ...
    const conditions = [
      this.#agentScopeCond("$1", agentId, opts.includePeerAgents === true),
