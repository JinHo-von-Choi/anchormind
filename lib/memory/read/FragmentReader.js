...
export class FragmentReader {
  ...
  async searchBySource(
    source, agentId, keyId, limit = 5, workspace = null, allWorkspaces = false
  ) {
    ...
    appendWorkspaceCondition(
      workspaceConditions, params, { workspace, allWorkspaces }, "workspace"
    );
    if (workspaceConditions.length > 0) query += ` AND ${workspaceConditions[0]}`;
