...
export class FragmentReader {
  ...
  async getById(id, agentId = "default", keyId = null, groupKeyIds = [], opts = {}) {
    ...
    keyFilter = opts.strictScope ? "" : keyScopeClause(params, "key_id", { keyId, groupKeyIds });
    let scopeFilter = "";
    if (opts.strictScope) {
      scopeFilter = exactScopeClause(params, "f", {
        keyId,
        workspace: opts.workspace ?? null
      });
    } else if (opts.workspace != null) {
      params.push(opts.workspace);
      scopeFilter = ` AND (f.workspace = $${params.length} OR f.workspace IS NULL)`;
    }
    ...
    const result = await queryWithAgentVector(agentId,
      `SELECT id, content, topic, keywords, type, importance, utility_score,
                    ...
                    case_id, goal, outcome, phase, resolution_status, assertion_status
             FROM ${SCHEMA}.fragments f WHERE ${baseWhere}${keyFilter}${scopeFilter}`,
      params
    );
  ...
  async getByIds(ids, agentId = "default", keyId = null, groupKeyIds = [], opts = {}) {
    ...
    const keyClause = opts.strictScope
      ? exactScopeClause(params, "f", { keyId, workspace: opts.workspace ?? null })
      : keyScopeClause(params, "key_id", { keyId, groupKeyIds });
  ...
  async getHistory(fragmentId, agentId = "default", keyId = null, groupKeyIds = [], opts = {}) {
    const historyOpts = { ...opts, strictScope: true };
    const current = await this.getById(fragmentId, agentId, keyId, groupKeyIds, historyOpts);

    /** current가 범위 밖이면 이력/체인도 조회하지 않는다. */
    if (!current) {
      return { current: null, versions: [], superseded_by_chain: [] };
    }

    const versionParams = [fragmentId];
    const versionScope = exactScopeClause(versionParams, "f", {
      keyId,
      workspace: opts.workspace ?? null
    });
    ...
    const versions = await queryWithAgentVector(agentId,
      `SELECT fragment_id, content, topic, keywords, type, importance,
       ...
       FROM ${SCHEMA}.fragment_versions v
       JOIN ${SCHEMA}.fragments f ON f.id = v.fragment_id
       WHERE v.fragment_id = $1${versionScope}
       ORDER BY amended_at DESC`,
      versionParams
    );

    const chainParams = [fragmentId];
    const sourceScope = exactScopeClause(chainParams, "source", {
      keyId,
      workspace: opts.workspace ?? null
    });
    const targetScope = exactScopeClause(chainParams, "target", {
      keyId,
      workspace: opts.workspace ?? null
    });
  ...
  async searchByKeywords(keywords, options = {}) {
    ...
    /** strict pilot은 key/workspace를 함께 정확히 제한하고, legacy만 그룹/global 호환을 유지한다. */
    if (options.strictScope) {
      conditions.push(exactScopeClause(params, "f", {
        keyId: options.keyId,
  ...
  async searchBySemantic(queryEmbedding, opts = {}) {
    ...
    /** API 키 격리 필터 */
    if (opts.strictScope) {
      conditions.push(exactScopeClause(params, "f", { keyId, workspace }).replace(/^ AND /, ""));
      paramIdx = params.length + 1;
    } else {
      if (keyId) {
   ...
   */
  async findCaseIdBySessionTopic(sessionId, topic, keyId = null, groupKeyIds = [], workspace = null, opts = {}) {
    const params     = [sessionId, topic];
    const keyClause  = opts.strictScope
      ? exactScopeClause(params, "f", { keyId, workspace })
      : keyScopeClause(params, "f.key_id", { keyId, groupKeyIds }) +
        (workspace != null ? ` AND (f.workspace = $${params.push(workspace)} OR f.workspace IS NULL)` : "");
    const { rows } = await queryWithAgentVector("default",
      `SELECT f.case_id FROM ${SCHEMA}.fragments f
        WHERE f.session_id = $1 AND f.topic = $2
          AND f.case_id IS NOT NULL AND f.valid_to IS NULL
          ${keyClause}
        LIMIT 1`,
      params
    );
   ...
   */
  async findErrorFragmentsBySessionTopic(sessionId, topic, keyId = null, groupKeyIds = [], workspace = null, opts = {}) {
    const params     = [sessionId, topic];
    const keyClause  = opts.strictScope
      ? exactScopeClause(params, "f", { keyId, workspace })
      : keyScopeClause(params, "f.key_id", { keyId, groupKeyIds }) +
        (workspace != null ? ` AND (f.workspace = $${params.push(workspace)} OR f.workspace IS NULL)` : "");
    const { rows } = await queryWithAgentVector("default",
      `SELECT f.id FROM ${SCHEMA}.fragments f
        WHERE f.session_id = $1 AND f.topic = $2
          AND f.type = 'error' AND f.valid_to IS NULL
          ${keyClause}
        LIMIT 10`,
      params
    );
   ...
   */
  async searchBySource(source, agentId, keyId, limit = 5, workspace = null, opts = {}) {
    let query = `SELECT id, content, topic, type, keywords, importance, source,
                        agent_id, created_at, is_anchor, access_count, accessed_at,
                        context_summary, session_id, workspace,
                        case_id, goal, outcome, phase, resolution_status, assertion_status
                 FROM ${SCHEMA}.fragments f
                 WHERE f.source = $1 AND (f.agent_id = $2 OR f.agent_id = 'default') AND f.valid_to IS NULL`;
    const params = [source, agentId || "default"];
    if (opts.strictScope) {
      query += exactScopeClause(params, "f", { keyId, workspace });
    } else {
      if (keyId) {
        params.push(keyId);
        query += ` AND f.key_id = $${params.length}`;
      }
      if (workspace) {
        params.push(workspace);
        query += ` AND (f.workspace = $${params.length} OR f.workspace IS NULL)`;
      }
    }
