...
export class FragmentReader {
   ...
   * @param {Object}      options - workspace/includePeerAgents/isAnchor 필터
   * @returns {Promise<Object[]>} 파편 배열
   */
  async searchBySource(source, agentId, keyId, limit = 5, options = {}) {
    /** 이전 내부 호출의 5번째 문자열(workspace)도 하위 호환한다. */
    const opts = typeof options === "string" ? { workspace: options } : (options || {});
    ...
    let query = `SELECT id, content, topic, type, keywords, importance, source,
                        ...
                        case_id, goal, outcome, phase, resolution_status, assertion_status
                 FROM ${SCHEMA}.fragments
                 WHERE source = $1 AND ${this.#agentScopeCond("$2", opts.includePeerAgents === true)} AND valid_to IS NULL`;
