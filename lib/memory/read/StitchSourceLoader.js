...
export async function fetchCausalLinks(fragmentIds, scope = {}, perFragment = 3) {
  ...
  appendWorkspaceCondition(workspaceConditions, params, scope, "f.workspace");
  const workspaceClause = workspaceConditions.length > 0
    ? ` AND ${workspaceConditions[0]}`
    : "";
  let agentClause = "";
  if (scope.includePeerAgents !== true) {
    params.push(scope.agentId || "default");
    agentClause = ` AND (f.agent_id = $${params.length} OR f.agent_id = 'default')`;
  }
       ...
       FROM (
         SELECT fl.from_id AS anchor_id, fl.to_id AS other_id, fl.relation_type,
          ...
          WHERE fl.from_id = ANY($1::text[])
            ...
            AND f.valid_to IS NULL${keyClause}${workspaceClause}${agentClause}
         UNION ALL
