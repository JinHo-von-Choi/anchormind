...
export async function fetchLinkedFragments(fragmentIds, scope = {}) {
  ...
  const strictScope = scope.strictScope === true;
  const sourceScope = strictScope
    ? exactScopeClause(params, "source", scope)
    : "";
  const targetScope = strictScope
    ...
    `SELECT sub.from_id, sub.to_id, sub.relation_type, sub.preview
       ...
       CROSS JOIN LATERAL (
          ...
          WHERE fl.from_id = fid.id
            ...
            AND source.valid_to IS NULL${sourceScope}${targetScope}
