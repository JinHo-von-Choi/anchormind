...
export class FragmentSearch {
  ...
  async _searchL2(query, excludeIds = [], agentId = "default", keyId = null, timeRange = null) {
    ...
    /** isAnchor-only/type 조합은 keyword/topic 진입점이 없으므로 시간 범위 reader를
     *  메타데이터 조회 경로로 재사용한다. SQL의 is_anchor 조건과 최종 SearchScope가
     *  동일한 3상태 계약을 이중으로 보장한다. text 쿼리는 L3 의미 검색이 담당한다. */
    ...
    if (
      ...
      results = await this.store.searchByTimeRange(
        timeRange?.from ?? null,
        timeRange?.to ?? null,
        { ...options, orderByImportance: true }
      );
    }
