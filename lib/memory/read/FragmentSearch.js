...
export class FragmentSearch {
  ...
  async search(query) {
    ...
    if (clean.length > 0) {
      ...
      this.store.touchLinked(accessIds, sq.agentId, sq.keyId, {
        ...
        includePeerAgents: sq.includePeerAgents
      }).catch(() => {});
  ...
  async _searchL2(query, excludeIds = [], agentId = "default", keyId = null, timeRange = null) {
    ...
    /** 조건 없는 최근 기억 조회는 L1 getRecent만 의존하지 않는다.
     *  L1 상위 ID가 다른 workspace로 가득 차 HotCache scope에서 전부 탈락해도,
     *  SQL workspace 조건으로 허용 범위의 최근 파편을 다시 채운다. */
    const hasSearchPredicate = (query.keywords?.length > 0) || query.topic || query.text;
    if (results.length === 0 && !hasSearchPredicate) {
      results = await this.store.searchByTimeRange(null, null, options);
    }
