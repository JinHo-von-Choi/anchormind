...
export class FragmentSearch {
  ...
  async search(query) {
    ...
    /** 검색 결과 직후의 부작용(검색 이벤트 영속화, adaptor 학습)을 단일 단계로 분리한다.
     *  searchEventId는 tool_feedback FK 연결을 위해 동기 await로 반환된다. */
    const searchEventId = await this._commitSearchSideEffects(query, sq, clean, {
      searchPath,
  ...
  /**
   * 검색 파이프라인 직후의 부작용을 분리된 단계로 처리한다.
   *
   * 책임:
   *  1. recordSearchEvent — 검색 이벤트 영속화. 동기 await으로 searchEventId를 반환하여
   *     호출자가 응답에 _searchEventId를 부착할 수 있게 한다(tool_feedback FK 계약).
   *  2. SearchParamAdaptor.recordOutcome — adaptor 학습 신호. fire-and-forget.
   *
   * 본 메서드는 후속 PR에서 SearchSideEffects 모듈로 외부화하기 위한 1차 분리다.
   * 검색 파이프라인(`_executeSearch`)은 본 메서드 호출 없이도 결과 자체를 만들 수 있다.
   *
   ...
   */
  async _commitSearchSideEffects(query, sq, cleanResult, ctx) {
