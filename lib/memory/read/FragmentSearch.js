...
export class FragmentSearch {
  ...
  async search(query) {
    ...
    /** Final pilot defense: every layer must return the exact scalar key/workspace tuple. */
    if (sq.strictScope === true) {
      clean = clean.filter(f => f?.key_id === sq.keyId && f?.workspace === sq.workspace);
    }
    ...
    if (clean.length > 0) {
      ...
      this.store.touchLinked(accessIds, sq.agentId, sq.keyId, sq.workspace, {
        strictScope: sq.strictScope === true
      }).catch(() => {});
  ...
  _buildSearchQuery(query) {
    const sq = {
      ...
      strictScope       : query.strictScope === true
    };
  ...
  async _buildTextRRF(sq, l1Ids, l1MissIds, l1IsFallback, cached, agentId, keyId, timeRange, searchPath, layerLatency) {
    ...
    const graphResults = sq.strictScope === true
      ? rawGraphResults.filter(f => f.key_id === keyId && f.workspace === sq.workspace)
      : (scope.isNoop() ? rawGraphResults : rawGraphResults.filter(f => scope.applyTo(f)));
  ...
  async _searchTemporal(sq) {
    ...
    return this.store.searchByTimeRange(
      sq.timeRange.from, sq.timeRange.to,
      {
        ...
        ...(sq.strictScope === true ? { strictScope: true } : {}),
  ...
  async _tryHotCache(ids, keyId = null, scope = null) {
    ...
    if (scope?.strictScope === true) {
      return valid.filter(f => f.key_id === scope.keyId && f.workspace === scope.workspace);
    }
